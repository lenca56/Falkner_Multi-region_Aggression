...
# Filtering only aggression moments
aggression_label = 1

for ind in range(len(featuresList)): # for each feature separately

    # fitting K-fold
    _, train_mse, test_mse = fit_KFold_linear_Gaussian_smoothing_all_days(animal=animal, group=group, features=[featuresList[ind]], circular_features=[circularList[ind]], region=region, Nbin_values=Nbin_values, alpha_values=alpha_values, behavioral_filter=aggression_label, K=K, blocks=400, path=data_path)
    ...
    indices_all, _ = index_filter_design_matrices_for_specific_behaviors(animal, group, behavior_label=aggression_label, path=data_path)
...
# saving
np.savez(f'../data/{animal}/{animal}_{group}_KFold={K}_MAP-estimation_AggOnly_region={region}', W_map_best=W_map_best, train_mse=train_mse_mean, test_mse=test_mse_mean, best_ind=best_ind, r2_best=r2_best)
