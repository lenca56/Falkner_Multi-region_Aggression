...
groupsAllNew = ['obs' for i in range(len(animalsObs))] + ['toy' for i in range(len(animalsToy))]
...
Y_all_without = np.empty((len(featuresList)), dtype=object)
X_all_without = np.empty((len(featuresList)), dtype=object)
Y_group_without = np.empty((len(featuresList)), dtype=object)
...
#                     animal, group=group, features=[featuresList[ind_feature]],
#                     bin_edges=bin_edges, path=data_path)
#                     Ytemp = np.array(temp_df[region])[mask_tmp]
...
#                 Y_all_without[ind_feature] = np.array(temp_df[region])
#                 Y_group_without[ind_feature] = np.array(temp_df[region])
#             else:
...
# Per-feature accumulators (lists; one entry per feature, each variable-length)
